...
class CustomDPOTrainer(DPOTrainer):
    ...
    def get_batch_loss_metrics(
        ...
        metrics[f"{prefix}rewards/originalloss"] = losses.cpu().mean()

        batch_size = batch["input_ids"].size(0) // 2
        chosen_labels, _ = batch["labels"].split(batch_size, dim=0)
        loss_delta = self.sft_loss(policy_chosen_logits, chosen_labels)
        metrics[f"{prefix}rewards/sftloss"] = loss_delta.cpu().mean()

        if self.ftx_gamma > 1e-6:
            losses += self.ftx_gamma * loss_delta
