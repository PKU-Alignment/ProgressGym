...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def __init__(
        ...
        ppo_config = PPOConfig(
            ...
            # "deepspeed_plugin": DeepSpeedPlugin(hf_ds_config=training_args.deepspeed) if training_args.deepspeed else None},
            log_with=(
                training_args.report_to[0]
                if training_args.report_to is not None
                else None
            ),
            project_kwargs={"logging_dir": training_args.logging_dir},
            kl_penalty="abs",  # added into original Llama_factory; default value is 'kl'
            # init_kl_coef=0.025, # added into original Llama_factory; default value is 0.2
            # adap_kl_ctrl=False, # added into original Llama_factory; default value is True
        )
        ...
        if finetuning_args.reward_model_type == "full":
            if self.is_deepspeed_enabled:
                if not (
                    ...
                    # config = deepcopy(self.accelerator.state.deepspeed_plugin.deepspeed_config)
