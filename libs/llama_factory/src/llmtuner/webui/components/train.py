...
def create_train_tab(engine: "Engine") -> Dict[str, "Component"]:
    ...
    with gr.Row():
        ...
        gradient_accumulation_steps = gr.Slider(
            value=8, minimum=1, maximum=1024, step=1
        )
    ...
    with gr.Accordion(label="Freeze config", open=False) as freeze_tab:
        with gr.Row():
            num_layer_trainable = gr.Slider(
                value=3, minimum=1, maximum=128, step=1, scale=2
            )
    ...
    with gr.Accordion(label="GaLore config", open=False) as galore_tab:
        with gr.Row():
            ...
            galore_update_interval = gr.Slider(
                value=200, minimum=1, maximum=1024, step=1, scale=2
            )
            galore_scale = gr.Slider(
                value=0.25, minimum=0, maximum=1, step=0.01, scale=2
            )
