...
class LifelongRLHFExaminee(ExamineeBase):
    independent: bool
    ...
    def reset(self, **kwargs):
        ...
        self.ppo_data: Union[Data, datasets.Dataset] = (
            default_ppo_data.copy()
            if "ppo_data" not in kwargs
            else Data(kwargs["ppo_data"])
        )
    ...
    def run(self, judge: JudgeBase) -> Iterable:
        if isinstance(judge, PredictJudge):
            warnings.warn(
                "LifelongRLHFExaminee is not designed for PredictJudge, and the result on PredictJudge should only be seen as a trivial baseline. Use ExtrapolativeRLHFExaminee instead."
            )
        ...
        while True:
            print("Running RLHF at timestep ", judge.current_timestep)
            try:
                rw_data = Data(
