...
def start_inference_backend(model_repoid_or_path: str, 
                            backend_type: Literal["sglang", "vllm"] = "sglang",
                            purpose: Literal["responses", "logprobs"] = "logprobs", 
                            silent: bool = True, 
                            port: int = PORT_NUM,
                            num_gpus: int = None,
                            template_type: Literal["auto", "alpaca", "mistral"] = "auto") -> Tuple[subprocess.Popen, Callable]:
    ...
    if backend_type == 'vllm':
        ...
        def vllm_process_batch(sample_dicts: List[dict], temperature: float = 0.2, max_tokens: int = 1024) -> List[dict]:
...
def dict_to_dialogue_list(dic: Union[dict, List[dict]]) -> Union[List[Dict[str, str]], List[List[Dict[str, str]]]]:
...
def fill_in_QA_template(
    ...
    if model_repoid_or_path == "alpaca":
        if suffix:
            warnings.warn(f"Suffix not supported except with mistral template. Ignoring suffix.")
...
\"\"\""""
        ...
        input_full = tokenizer.apply_chat_template(prompt, tokenize=False, add_generation_prompt=True)
