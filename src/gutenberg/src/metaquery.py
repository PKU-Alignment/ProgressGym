...
class meta_query(object):
    ...
    def __init__(self, path="../metadata/metadata.csv", filter_exist=True):
        ...
        if filter_exist == True:  ## filter the books for which we have the data
            path_text = os.path.abspath(
                os.path.join(path, os.pardir, os.pardir, "data", "text")
            )
            list_files = []
    ...
    def get_lang(self):
        list_lang = [
            [k for k in h.strip("[]")[1:-1].replace("', '", "_").split("_")]
            for h in self.df["language"].dropna()
        ]
    ...
    def get_subjects(self):
        list_subjects = [
            [k for k in h.strip("{}")[1:-1].replace("', '", "_").split("_")]
            for h in self.df["subjects"].replace("set()", np.nan).dropna()
        ]
    ...
    def filter_subject(self, subject_sel, how="only"):
        ...
        if how == "only":
            ...
            s = self.df[
                self.df["subjects"]
                .str.contains("'%s'" % (re.escape(subject_sel)))
                .replace(np.nan, False)
            ]
        else:
            s = meta
    ...
    def filter_year(self, y_sel, hmin=20):
        ...
        if isinstance(y_sel, (list, np.ndarray)):
            s = self.df[
                (self.df["authoryearofbirth"] <= y_sel[1] - hmin)
                & (self.df["authoryearofdeath"] > y_sel[0])
            ]
        else:
            s = self.df[
                (self.df["authoryearofbirth"] <= y_sel - hmin)
                & (self.df["authoryearofdeath"] > y_sel)
            ]
        self.df = s
    ...
    def filter_author(self, s_sel):
        s = self.df[
            self.df["author"]
            .str.contains(re.escape(s_sel), case=False)
            .replace(np.nan, False)
        ]
        self.df = s
