...
def make_df_metadata(
    path_xml="../metadata/rdf-files.tar.bz2",
...
def parsemetadata(ebook):
    ...
    if title is not None:
        result["title"] = fixsubtitles(safeunicode(title.text, encoding="utf-8"))
    # subject lists
    ...
    # formats
    result["formats"] = {
        file.find("{%(dc)s}format//{%(rdf)s}value" % NS).text: file.get(
            "{%(rdf)s}about" % NS
        )
